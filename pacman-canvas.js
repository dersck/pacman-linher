...
function geronimo() {
	...
	var ajax_add = (n, s, l) => {
		...
		$.ajax({
			...
			success: function (data) {
				...
				$('#highscore-form').html('<span class="button" id="show-highscore">Ver Highscore List</span>');
			},
	...
	function addHighscore() {
		...
		$("#highscore-form").html("Guardando highscore...");
	...
	function Game() {
		...
		this.newGame = function () {
			var r = confirm("¿Seguro que quieres reiniciar?");
			if (r) {
		...
		this.nextLevel = function () {
			...
			if (this.level === FINAL_LEVEL) {
				console.log('siguiente nivel, ' + FINAL_LEVEL + ', end game');
				game.endGame(true);
				game.showHighscoreForm();
			} else {
				this.level++;
				console.log("Level " + game.level);
				game.pauseAndShowMessage("Nivel " + game.level, this.getLevelTitle() + "<br/>(Clic para continuar)");
		...
		this.showHighscoreForm = function () {
			...
			var inputHTML = scoreIsValid ? `<div id='highscore-form'>
				...
				</div>` : `<div id='invalid-score'>Tu puntuación parece falsa, la lista de mejores puntuaciones es sólo para jugadores honestos. ;)</div>`;
			this.pauseAndShowMessage("Fin del juego", "Puntaje Total: " + this.score.score + (HIGHSCORE_ENABLED ? inputHTML : ''));
			$('#playerName').focus();
		}
		...
		this.forcePause = function () {
			this.timer.stop();
			this.pauseAndShowMessage("Pausa", "Clic para Reanudar");
		}
		...
		this.pauseResume = function () {
			if (this.gameOver) {
				console.log('No se puede pausar / reanudar. GameOver ajustado a true.');
				return;
			}
			if (!this.started) {
		...
		this.init = async (state) => {
			...
			if (pinky === null || pinky === undefined) {
				pinky = new Ghost(GHOSTS.PINKY, 7, 5, 'img/goliat.svg', 2, 2);
				inky = new Ghost(GHOSTS.INKY, 8, 5, 'img/essential.svg', 13, 11);
				blinky = new Ghost(GHOSTS.BLINKY, 9, 5, 'img/plus.svg', 13, 0);
				clyde = new Ghost(GHOSTS.CLYDE, 10, 5, 'img/express.svg', 2, 11);
			} else {
				pinky.reset();
				inky.reset();
				blinky.reset();
	...
	$(document).ready(function () {
		...
		$('body').on('click', '#score-submit', function () {
			...
			if ($('#playerName').val() === "" || $('#playerName').val() === undefined) {
				$('#form-validator').html("Ingresa un nombre<br/>");
			} else {
				$('#form-validator').html("");
				addHighscore();
			}
